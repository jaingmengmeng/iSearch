...
class QueryRewrite():
    ...
    def __init__(self, stopwords_path, file_path=None, dict_path="./resources/word_freq.json",
                 model_path='./model/word2vec.model', cn_dict_path="./resources/cn_dict.txt"):
        self.file_path = file_path
        self.stopwords_path = stopwords_path
        self.model_path = model_path
        self.dict_path = dict_path
        self.segmenter = Word_Segment(stopwords_path)
        self.autocorrecter = AutoCorrecter(self.segmenter, self.dict_path, self.file_path, cn_dict_path)
    ...
    def comprehensive_extract(self, query):
        ...
        associate_words = list(map(lambda x: x[0], self.model.wv.most_similar(words, topn=100)))
        # print(similar_words)
        return (corrected_query, associate_words, " ".join(words + similar_words))


# 输入你的停用词文件位置、词频词典位置、所有doc的位置以及模型位置
# qr = QueryRewrite(stopwords_path='./resources/stopwords.txt', file_path="./files",
#                   dict_path="./resources/word_freq.json", model_path='./model/word2vec.model',
